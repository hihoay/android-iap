...
auto_create_tag(){
  ...
  if [ "$BRANCH_NAME" == "develop" ] || [ "$BRANCH_NAME" == "release" ] || [ "$BRANCH_NAME" == "main" ] || [ "$BRANCH_NAME" == "testing" ]; then
      ...
      title="[$BRANCH_NAME] build complete!"
      message="[Tag: $newTag]($BUILD_URL/pipeline-console)"
      url="$BUILD_URL/pipeline-console"
      color="green"
      echo $title
      echo $message
      echo $url
      echo $color
      echo $newTag
      curl -H "Content-Type: application/json" -X POST -d "{\"content\": \"[$BRANCH_NAME] $message \"}" "$DISCORD_WEBHOOK"
