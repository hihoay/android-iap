...
echo "--------------------------------"
    ...
    unzip /root/commandlinetools.zip -d /root/android-sdk/cmdline-tools
    ...
    echo "export PATH=/root/android-sdk/cmdline-tools/cmdline-tools/bin/:/$PATH" >> /root/.bashrc
    # shellcheck disable=SC2129
    echo "export ANDROID_HOME=/root/android-sdk" >> /etc/profile
    echo "export PATH=/$ANDROID_HOME/cmdline-tools/latest/bin:/$ANDROID_HOME/platform-tools:/$PATH" >> /etc/profile
    echo "export PATH=/root/android-sdk/cmdline-tools/latest/bin:/$PATH" >> /etc/profile
    source /root/.bashrc
    source /etc/profile
  ...
  if  [ "$current_branch" == "release" ] || [ "$current_branch" == "main" ] || [ "$current_branch" == "testing" ]; then
   ...
   for file in $(find app/build/outputs -type f \( -name "*.apk" -o -name "*.aab" \)); do
      ...
      echo "qr: $qr"
       ...
       Package: \`implementation(\"com.taymay:android-firebase:$newTag\")\`
       "
