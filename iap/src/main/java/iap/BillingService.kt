...
internal class BillingService(
    ...
    override fun getAllSubscription(productID: String, callback : (String?, DataWrappers.ProductDetails?) -> Unit) {
        ...
        mBillingClient.queryProductDetailsAsync(params) { billingResult, productDetailsList ->
            if (billingResult.responseCode == BillingClient.BillingResponseCode.OK) {
                val firstProductDetail = productDetailsList.firstOrNull()
                val dataWrapperProduct = firstProductDetail?.toDataWrapper()
                val offerId = firstProductDetail
                    ?.subscriptionOfferDetails
                    ?.firstOrNull()
                    ?.basePlanId // hoặc chọn theo điều kiện cụ thể
                Log.d("SUB_RESULT", "Product Detail:\n${dataWrapperProduct?.prettyToString()}")
                Log.d("SUB_RESULT", "Offer ID đã đăng ký (giả định): $offerId")
                callback.invoke(offerId, dataWrapperProduct)
            }
        }
    }
